...
function TestCasesIndex() {
  ...
  const [testPlans, setTestPlans] = useState<{id: number; name: string}[]>([])
  ...
  const fetchTestPlans = async (versionId: number) => {
    try {
      setTestPlansLoading(true)
      const response = await projectApi.getTestPlans(versionId)
  ...
  const handleVersionSelect = (version: ProjectVersion) => {
    ...
    setSelectedPlan(null)
    setTestPlans([])
    fetchTestPlans(version.id)
  }

  const handlePlanSelect = (plan: {id: number; name: string}) => {
    setSelectedPlan(plan)
    if (selectedProject && selectedVersion) {
      navigate(`/projects/${selectedProject.id}/versions/${selectedVersion.id}/test-plans/${plan.id}/test-cases`)
    }
  }
  ...
  const getBreadcrumb = () => {
    ...
    if (selectedVersion) parts.push(selectedVersion.version)
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-2xl font-bold">{t('sidebar.testCases', '测试用例')}</h1>
      </div>
      ...
      {!selectedProject ? (
        ...
        <div>
          <h2 className="text-lg font-semibold mb-4">
            {t('project.selectTestPlan', '选择测试计划')}
          </h2>
