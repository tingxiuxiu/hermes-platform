...
describe('ConfigDrawer (integration)', () => {
  beforeEach(() => {
    ...
    document.documentElement.classList.remove('light', 'dark', 'apple-light', 'apple-dark')
  ...
  it('opens the drawer and renders the sections', async () => {
    ...
    await expect.element(drawer.getByText(/^Sidebar$/i)).toBeInTheDocument()
  ...
  describe('theme selection', () => {
    ...
    it('applies apple-light theme to <html> and cookie', async () => {
      const screen = await renderConfigDrawer()
      await openDrawer(screen)
      await userEvent.click(screen.getByRole('radio', { name: /select apple light/i }))
      await vi.waitFor(() =>
        expect(document.documentElement.classList.contains('apple-light')).toBe(true)
      )
      expect(getCookie('vite-ui-theme')).toBe('apple-light')
    })

    it('applies apple-dark theme to <html> and cookie', async () => {
      const screen = await renderConfigDrawer()
      await openDrawer(screen)
      await userEvent.click(screen.getByRole('radio', { name: /select apple dark/i }))
      await vi.waitFor(() =>
        expect(document.documentElement.classList.contains('apple-dark')).toBe(true)
      )
      expect(getCookie('vite-ui-theme')).toBe('apple-dark')
    })
